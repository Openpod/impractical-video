...
- CHARACTER AESTHETIC DEFAULT: for character-driven work where the user has NOT specified a conflicting appearance, favor polished, high-production-value, conventionally attractive adult character design. For adult women specifically, "attractive", "beautiful", "sexy", "sensual", "alluring", "model-like", or "fashion-model beauty" means unmistakably adult late-20s-to-30s supermodel-level glamour by default: striking facial beauty, sculpted cheekbones, expressive eyes, full lips, polished hair and makeup, voluptuous hourglass physique, full bust, defined waist, shapely hips, confident pose, tasteful neckline/bust emphasis through wardrobe and posture, stylish fitted clothing, cinematic beauty lighting, and sensual but composed body language. This is only a DEFAULT: explicit user intent always wins (age, body type, modesty, realism level, wardrobe, cultural/aesthetic direction). HARD RULE — this default never applies to characters who are minors, teens, ambiguous-age, childlike, or otherwise youth-coded: those are designed without any sensualization, full stop.
- ADULT FEMALE PORTFOLIOS: the portfolio is where this look must be locked. When generating or revising an adult female character portfolio with attractive/sensual/glamour intent, load adult-glamour-portfolio before writing reference.md or calling generateReferencePortfolio. Write affirmative asset-specific prompts with hard visual language, not euphemisms: supermodel-level face, voluptuous hourglass body, full bust, defined waist, shapely hips, confident sensual posture, and the exact wardrobe requested.
...
- SKILLS BY STEP — readSkill loads the method. When the step below comes up, readSkill the named skill BEFORE doing that step:
	  ...
	  - Creating or revising adult female character references or portfolios where the user asks for attractive, beautiful, sexy, sensual, alluring, glamorous, model-like, curvy, busty, voluptuous, fashionable, or supermodel-coded appearance → readSkill adult-glamour-portfolio BEFORE writing reference.md or generating the portfolio.
  ...
  return {
    ...
    generateReferencePortfolio: tool({
      ...
      execute: async ({ aspect_ratio, category, conditioning_image_ids, conditioning_image_urls, prompt, reference_id, style_image_ids, title }) => {
        ...
        const assetPrompt = enrichPortfolioAssetPrompt({ category, prompt });
