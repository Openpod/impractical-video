// Ported from opencut-classic's sounds search route for the in-process
// editor: host env replaces webEnv, and the upstash rate limiter is
// dropped (this app fronts it with its own auth/session).
...
function proxiedSoundUrl(url: string | undefined) {
	if (!url) return undefined;
	return `/api/sounds/audio?url=${encodeURIComponent(url)}`;
}
...
function transformFreesoundResult(result: z.infer<typeof freesoundResultSchema>) {
	...
	return {
		...
		url: result.url,
		previewUrl: proxiedSoundUrl(previewUrl),
