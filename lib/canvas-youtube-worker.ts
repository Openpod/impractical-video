...
import { getFalKey } from "@/lib/local-provider-settings";
import {
...
export type CanvasYoutubeImportPayload = {
  brief?: string | null;
  importId: string;
  projectId: string;
...
export type CanvasYoutubeGeneratePayload = {
  importId: string;
  projectId: string;
  userId: string;
};
...
async function configureFal() {
  const key = await getFalKey();
  if (!key) throw new Error("Add your fal.ai key in Account → API keys before generating.");
...
export async function runCanvasYoutubeGenerate(payload: CanvasYoutubeGeneratePayload) {
...
export async function runCanvasYoutubeImport(payload: CanvasYoutubeImportPayload) {
