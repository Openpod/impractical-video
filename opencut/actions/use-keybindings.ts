...
function isHostedEditorKeybindingsSuppressed(): boolean {
	const hostedScopes = document.querySelectorAll<HTMLElement>(
		".opencut-scope[data-host-editor-active]",
	);
	if (hostedScopes.length === 0) return false;
	return !Array.from(hostedScopes).some(
		(scope) => scope.dataset.hostEditorActive === "true",
	);
}
...
export function useKeybindingsListener() {
	...
	useEffect(() => {
		...
		const handleKeyDown = (ev: KeyboardEvent) => {
			...
			if (isHostedEditorKeybindingsSuppressed()) {
				return;
			}
			...
			if (boundAction === "copy-selected" && hasNativeTextSelection()) {
				return;
			}
