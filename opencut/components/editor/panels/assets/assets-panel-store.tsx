...
const HIDDEN_TAB_SET = new Set<Tab>(HIDDEN_TAB_KEYS);

export const VISIBLE_TAB_KEYS = TAB_KEYS.filter(
	(tab) => !HIDDEN_TAB_SET.has(tab),
);

export function isAssetsPanelTabVisible(tab: Tab): boolean {
	return !HIDDEN_TAB_SET.has(tab);
}

// Host addition: media grid type filter.
export type MediaTypeFilter = "all" | "audio" | "image" | "video";

const createHugeiconsIcon =
	({ icon }: { icon: IconSvgElement }) =>
	// eslint-disable-next-line react/display-name -- upstream factory (host lint only)
