...
export class ScenesManager {
	...
	getActiveScene(): TScene {
		if (!this.active) {
			// Host fix: while the host bridge reloads an agent-edited project the
			// active pointer is briefly unset; renders during that window used to
			// throw and crash the tree. Fall back to the main/first scene — only a
			// project with NO scenes is a genuinely broken state worth throwing on.
			const fallback = this.list.find((scene) => scene.isMain) ?? this.list[0];
