...
import { TICKS_PER_SECOND } from "@opencut/wasm/media-time";

/** How far ahead of a clip's start its decoder gets pre-warmed. */
const WARM_LOOKAHEAD_TICKS = 1.5 * TICKS_PER_SECOND;
...
async function resolveVideoNode({
	...
	if (clipTime < 0 || clipTime >= node.params.duration) {
		// Approaching clip: warm its decoder now. A cold first frame costs a
		// container parse + decoder init + keyframe seek (~300ms), which would
		// otherwise stall the serialized render loop right at the cut.
		if (clipTime < 0 && clipTime > -WARM_LOOKAHEAD_TICKS) {
			void videoCache.warmFrameAt({
				mediaId: node.params.mediaId,
				file: node.params.file,
				time: mediaTimeToSeconds({
					time: roundMediaTime({ time: node.params.trimStart }),
				}),
			});
		}
		return null;
