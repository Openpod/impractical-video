...
export function Timeline() {
	...
	useEffect(() => {
		...
		const onWheel = (e: WheelEvent) => {
			...
			// Host addition: with no vertical overflow, plain wheel means
			// horizontal — mouse users have no trackpad gesture for sideways.
			const hasVerticalOverflow = tracks.scrollHeight > tracks.clientHeight + 1;
			const isHorizontal =
				e.shiftKey ||
	...
	// Host addition: middle-mouse drag pans the timeline — mouse users have no
	// trackpad for horizontal scrolling (shift+wheel also works).
	useEffect(() => {
		const container = timelineRef.current;
		if (!container) return;
		let panning = false;
		...
		const onPointerMove = (e: PointerEvent) => {
			...
			const tracks = tracksScrollRef.current;
			if (!tracks) return;
			tracks.scrollLeft = Math.max(0, tracks.scrollLeft - (e.clientX - lastX));
			tracks.scrollTop = Math.max(0, tracks.scrollTop - (e.clientY - lastY));
			lastX = e.clientX;
			lastY = e.clientY;
			syncFollowers();
			saveScrollPositionRef.current();
