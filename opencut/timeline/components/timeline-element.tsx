...
import { addMediaTime, type MediaTime, roundMediaTime, TICKS_PER_SECOND } from "@opencut/wasm";
import {
	getActionDefinition,
	type TAction,
...
export function TimelineElement({
	...
	useEffect(() => {
		const clear = () => setSectionSelection(null);
		window.addEventListener(CLIP_SECTION_CLEAR_EVENT, clear);
		return () => window.removeEventListener(CLIP_SECTION_CLEAR_EVENT, clear);
	}, []);

	// Alt/Option+drag on a video element marks a section (source-time span)
	// and dispatches it to the host. Capture phase so the inner button's
	// select/drag handling never sees the gesture.
	const handleSectionMouseDown = (event: React.MouseEvent<HTMLDivElement>) => {
		// A plain click dismisses an existing section band (no composer is
		// currently wired to consume/clear it).
		if (!event.altKey && sectionSelection) setSectionSelection(null);
		if (!event.altKey || event.button !== 0 || element.type !== "video") return;
		event.preventDefault();
		event.stopPropagation();
		const container = event.currentTarget.getBoundingClientRect();
		const clamp = (x: number) => Math.max(0, Math.min(x, elementWidth));
		const startX = clamp(event.clientX - container.left);
		setSectionSelection({ endPx: startX, startPx: startX });
		// Scrub the player to the drag edge so the user SEES what they select.
		const scrubTo = (x: number) => {
			editor.playback.seek({
				time: roundMediaTime({
					time:
						displayedStartTime +
						(x / (timelinePixelsPerSecond || 1)) * TICKS_PER_SECOND,
				}),
			});
		};
		scrubTo(startX);
		const onMove = (move: MouseEvent) => {
			const x = clamp(move.clientX - container.left);
			setSectionSelection({ endPx: x, startPx: startX });
			scrubTo(x);
		};
		const onUp = (up: MouseEvent) => {
			window.removeEventListener("mousemove", onMove);
			window.removeEventListener("mouseup", onUp);
			const upX = clamp(up.clientX - container.left);
			const left = Math.min(startX, upX);
			const right = Math.max(startX, upX);
			if (right - left < 6) {
				setSectionSelection(null);
				return;
			}
			setSectionSelection({ endPx: right, startPx: left });
			const pixelsPerSecond = timelinePixelsPerSecond || 1;
			// trimStart is MediaTime TICKS; the px math is in seconds.
			const trimStart =
				"trimStart" in element && typeof element.trimStart === "number"
					? element.trimStart / TICKS_PER_SECOND
					: 0;
			window.dispatchEvent(
				new CustomEvent<ClipSectionEventDetail>(CLIP_SECTION_EVENT, {
					detail: {
						elementId: element.id,
						inSeconds: trimStart + left / pixelsPerSecond,
						mediaId: hasMediaId(element) ? element.mediaId : null,
						name: element.name,
						outSeconds: trimStart + right / pixelsPerSecond,
						trackId: track.id,
					},
				}),
			);
			...
			<ContextMenu>
				<ContextMenuTrigger asChild>
					<div
						...
						onMouseDownCapture={handleSectionMouseDown}
					>
						<ElementInner
							element={element}
						...
						{sectionSelection ? (
							<div
								className="pointer-events-none absolute top-0 z-10 rounded-sm"
								style={{
									left: `${Math.min(sectionSelection.startPx, sectionSelection.endPx)}px`,
									width: `${Math.abs(sectionSelection.endPx - sectionSelection.startPx)}px`,
									height: `${baseTrackHeight}px`,
									background: "rgba(239, 68, 68, 0.22)",
									boxShadow: "inset 0 0 0 1.5px rgba(239, 68, 68, 0.85)",
								}}
							/>
						) : null}
						{isSelected && (
...
// Section marking: Alt/Option+drag across a video element selects a time span
// and hands it to the host app (floating composer → chat agent) via a DOM
// event — the vendored timeline stays decoupled from the host's chat.
export const CLIP_SECTION_EVENT = "videofs:clip-section";
export const CLIP_SECTION_CLEAR_EVENT = "videofs:clip-section-clear";

export type ClipSectionEventDetail = {
	elementId: string;
	inSeconds: number;
	mediaId: string | null;
	name: string;
...
// Filmstrip generation: sample real frames across the clip's visible range so
// the strip shows the video progressing instead of tiling frame one. Strips
// are memoized per (media, trim, count) — seeking a <video> is expensive.
const filmstripCache = new Map<string, Promise<string | null>>();
/** Resolved strips per media+range, keyed by frame count — lets zoom show the
 * nearest already-built density instantly while the exact one builds. */
const filmstripVariants = new Map<string, Map<number, string>>();
const FILMSTRIP_CACHE_MAX = 60;

function buildFilmstrip(input: {
	end: number;
	frameCount: number;
	start: number;
	tileHeight: number;
	...
	return new Promise((resolve) => {
		...
		canvas.width = input.tileWidth * input.frameCount * scale;
		canvas.height = input.tileHeight * scale;
		...
		// A stalled video (bad codec, blocked range requests) must not wedge the
		// cache entry in a never-resolving state.
		const timeout = window.setTimeout(() => settle(null), 20_000);
		const finish = () => {
			try {
				settle(canvas.toDataURL("image/jpeg", 0.72));
			} catch {
				// Tainted canvas (no CORS on the media host) — caller falls back
				// to the single-thumbnail tiling.
				settle(null);
			}
		};
		let started = false;
		const seekNext = () => {
			if (slot >= input.frameCount) return finish();
			const span = Math.max(0, input.end - input.start);
			video.currentTime =
				input.start + ((slot + 0.5) / input.frameCount) * span;
		};
		const start = () => {
			if (started) return;
			started = true;
		...
		video.addEventListener("seeked", () => {
			...
			// Cover-crop each frame into its tile.
			const tileRatio = input.tileWidth / input.tileHeight;
			const sourceRatio = sourceWidth / sourceHeight;
			const cropWidth = sourceRatio > tileRatio ? sourceHeight * tileRatio : sourceWidth;
			const cropHeight = sourceRatio > tileRatio ? sourceHeight : sourceWidth / tileRatio;
			context.drawImage(
				video,
				...
				cropWidth,
				cropHeight,
				slot * input.tileWidth * scale,
				0,
				input.tileWidth * scale,
				input.tileHeight * scale,
			);
			slot += 1;
			seekNext();
		});
		...
		// Listeners are attached BEFORE src: a cached video can fire
		// loadedmetadata during the same task as the src assignment, and
		// missing it left the strip permanently on the fallback thumbnail.
...
function useVideoFilmstrip(input: {
	end: number;
	enabled: boolean;
	frameCount: number;
	mediaId: string;
...
}): { frameCount: number; url: string } | null {
	const [strip, setStrip] = useState<{ frameCount: number; url: string } | null>(null);
	const baseKey = input.url
		? `${input.mediaId}:${input.start.toFixed(1)}:${input.end.toFixed(1)}:${Math.round(input.tileHeight)}`
		: null;
	const key = baseKey ? `${baseKey}:${input.frameCount}` : null;
	useEffect(() => {
		if (!input.enabled || !baseKey || !key || !input.url) return;
		let alive = true;
		// Zoom must feel continuous: immediately show the nearest ALREADY-BUILT
		// density for this media (drawn at natural tile width, so it is never
		// distorted), then let the exact density replace it when it resolves.
		const variants = filmstripVariants.get(baseKey);
		if (variants?.size) {
			let nearest: number | null = null;
			for (const count of variants.keys()) {
				if (count === input.frameCount) {
					nearest = count;
					break;
				}
				if (
					nearest === null ||
					Math.abs(count - input.frameCount) < Math.abs(nearest - input.frameCount)
				) {
					nearest = count;
				}
			}
			if (nearest !== null) {
				const url = variants.get(nearest)!;
				setStrip((current) =>
					current?.url === url && current.frameCount === nearest
						? current
						: { frameCount: nearest, url },
				);
				if (nearest === input.frameCount) return;
			}
		}
		let pending = filmstripCache.get(key);
		if (!pending) {
			pending = buildFilmstrip({
				end: input.end,
			...
			if (filmstripCache.size > FILMSTRIP_CACHE_MAX) {
				const oldest = filmstripCache.keys().next().value;
				if (oldest) filmstripCache.delete(oldest);
			}
		}
		void pending.then((result) => {
			// A failed build stays out of the cache so a later mount retries.
			if (result === null) {
				filmstripCache.delete(key);
				return;
			}
			let byCount = filmstripVariants.get(baseKey);
			if (!byCount) {
				byCount = new Map();
				filmstripVariants.set(baseKey, byCount);
				if (filmstripVariants.size > FILMSTRIP_CACHE_MAX) {
					const oldest = filmstripVariants.keys().next().value;
					if (oldest) filmstripVariants.delete(oldest);
				}
			}
			byCount.set(input.frameCount, result);
			if (alive) setStrip({ frameCount: input.frameCount, url: result });
		});
		return () => {
			alive = false;
		};
		// eslint-disable-next-line react-hooks/exhaustive-deps
	}, [input.enabled, baseKey, key, input.url]);
	return strip;
}
...
function TiledMediaContent({
	...
	// Sampled filmstrip across the clip's visible (trimmed) range — roughly one
	// frame per 1.5s, so the strip shows the video progressing. MediaTime is
	// integer TICKS: convert to seconds before anything touches currentTime,
	// or every seek clamps to the video's end (a strip of the last frame).
	const isVideo = element.type === "video";
	const visibleStart = isVideo ? element.trimStart / TICKS_PER_SECOND : 0;
	const visibleEnd = isVideo
		? Math.max(visibleStart, (element.duration - element.trimEnd) / TICKS_PER_SECOND)
		: 0;
	// Frame count follows the RENDERED width (zoom included) so the strip
	// always spans the element instead of looping. Rounded UP to even counts:
	// the strip is generated at (or past) the element width and the tail is
	// cropped, so tiles keep their natural width — never stretched.
	const idealCount = elementWidthPx
		? elementWidthPx / tileWidth
		: (visibleEnd - visibleStart) / 1.5;
	const frameCount = Math.min(60, Math.max(2, 2 * Math.ceil(Math.ceil(idealCount) / 2)));
	const filmstrip = useVideoFilmstrip({
		enabled: isVideo && Boolean(mediaAsset?.url),
		end: visibleEnd,
		frameCount,
		mediaId: element.mediaId,
		start: visibleStart,
		tileHeight: trackHeight,
		tileWidth,
		url: isVideo ? mediaAsset?.url : null,
	});

	if (!imageUrl && !filmstrip?.url) {
		return (
			<span className="text-foreground/80 truncate text-xs">
				{element.name}
			</span>
		...
		<>
			<div
				...
				style={{
					...
					backgroundImage: `url(${filmstrip?.url ?? imageUrl})`,
					// The strip renders at natural tile width and overshoots the
					// element; the container crops the tail — no stretch, no loop.
					// Sized by the RESOLVED strip's own count (never the requested
					// one), so an interim density is never scaled/elongated.
					backgroundRepeat: filmstrip ? "no-repeat" : "repeat-x",
					backgroundSize: filmstrip
						? `${tileWidth * filmstrip.frameCount}px ${trackHeight}px`
...
function ElementContent({ element, elementWidthPx, track }: ElementContentProps) {
	switch (element.type) {
		case "text":
