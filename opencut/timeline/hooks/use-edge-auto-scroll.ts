...
export function useEdgeAutoScroll({
	...
	useEffect(() => {
		...
		// Host fix: a stationary click near a viewport edge must not scroll —
		// only auto-scroll once the pointer has actually dragged.
		const startMouseX = getMouseClientX();
		...
		const step = () => {
			...
			if (!dragUnlocked) {
				if (Math.abs(getMouseClientX() - startMouseX) > 6) {
					dragUnlocked = true;
				} else {
					rafRef.current = requestAnimationFrame(step);
					return;
